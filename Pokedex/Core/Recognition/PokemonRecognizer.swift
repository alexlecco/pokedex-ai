import Vision
import UIKit
import Foundation

// MARK: - Recognition Result
struct RecognitionResult: Sendable {
    let pokemonId: Int
    let pokemonName: String
    let confidence: Float
}

// MARK: - Pokemon Recognizer
// Uses VNGenerateImageFeaturePrintRequest to compare camera frames
// against pre-built feature vectors of all 151 Gen 1 Pokémon sprites.
// No custom CoreML model required — uses Apple's built-in feature extraction.
actor PokemonRecognizer {
    static let shared = PokemonRecognizer()

    private var featureDatabase: [Int: VNFeaturePrintObservation] = [:]
    private var isBuilding = false
    private var isDatabaseReady = false

    private init() {}

    // MARK: - Build feature database from PokeAPI sprites
    func buildDatabase(progress: @escaping @Sendable (Int, Int) -> Void) async {
        guard !isDatabaseReady, !isBuilding else { return }
        isBuilding = true

        for id in 1...151 {
            guard let url = URL(string: "https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/other/official-artwork/\(id).png"),
                  let data = try? Data(contentsOf: url),
                  let image = UIImage(data: data),
                  let cgImage = image.cgImage else {
                await MainActor.run { progress(id, 151) }
                continue
            }

            if let observation = await extractFeaturePrint(from: cgImage) {
                featureDatabase[id] = observation
            }
            await MainActor.run { progress(id, 151) }
        }

        isBuilding = false
        isDatabaseReady = !featureDatabase.isEmpty
    }

    // MARK: - Recognize Pokémon from camera frame
    func recognize(pixelBuffer: CVPixelBuffer) async -> RecognitionResult? {
        guard isDatabaseReady else { return nil }

        let handler = VNImageRequestHandler(cvPixelBuffer: pixelBuffer, options: [:])
        let request = VNGenerateImageFeaturePrintRequest()

        do {
            try handler.perform([request])
        } catch {
            return nil
        }

        guard let queryPrint = request.results?.first as? VNFeaturePrintObservation else {
            return nil
        }

        return findClosestMatch(to: queryPrint)
    }

    // MARK: - Recognize from UIImage (for photo capture)
    func recognize(image: UIImage) async -> RecognitionResult? {
        guard isDatabaseReady, let cgImage = image.cgImage else { return nil }

        let handler = VNImageRequestHandler(cgImage: cgImage, options: [:])
        let request = VNGenerateImageFeaturePrintRequest()

        do {
            try handler.perform([request])
        } catch {
            return nil
        }

        guard let queryPrint = request.results?.first as? VNFeaturePrintObservation else {
            return nil
        }

        return findClosestMatch(to: queryPrint)
    }

    var databaseReady: Bool { isDatabaseReady }
    var databaseSize: Int { featureDatabase.count }

    // MARK: - Private helpers
    private func extractFeaturePrint(from cgImage: CGImage) async -> VNFeaturePrintObservation? {
        let handler = VNImageRequestHandler(cgImage: cgImage, options: [:])
        let request = VNGenerateImageFeaturePrintRequest()
        try? handler.perform([request])
        return request.results?.first as? VNFeaturePrintObservation
    }

    private func findClosestMatch(to query: VNFeaturePrintObservation) -> RecognitionResult? {
        var bestId: Int = -1
        var bestDistance: Float = .infinity

        for (id, print) in featureDatabase {
            var distance: Float = 0
            guard (try? print.computeDistance(&distance, to: query)) != nil else { continue }
            if distance < bestDistance {
                bestDistance = distance
                bestId = id
            }
        }

        guard bestId != -1 else { return nil }

        // Convert distance to confidence (lower distance = higher confidence)
        // Typical distances range 0.0 (identical) to ~1.5+ (very different)
        let confidence = max(0, 1.0 - bestDistance)
        let name = gen1Names[bestId] ?? "pokemon-\(bestId)"

        return RecognitionResult(
            pokemonId: bestId,
            pokemonName: name,
            confidence: confidence
        )
    }
}

// MARK: - Gen 1 Names lookup (avoids extra API calls during recognition)
let gen1Names: [Int: String] = [
    1: "bulbasaur", 2: "ivysaur", 3: "venusaur", 4: "charmander", 5: "charmeleon",
    6: "charizard", 7: "squirtle", 8: "wartortle", 9: "blastoise", 10: "caterpie",
    11: "metapod", 12: "butterfree", 13: "weedle", 14: "kakuna", 15: "beedrill",
    16: "pidgey", 17: "pidgeotto", 18: "pidgeot", 19: "rattata", 20: "raticate",
    21: "spearow", 22: "fearow", 23: "ekans", 24: "arbok", 25: "pikachu",
    26: "raichu", 27: "sandshrew", 28: "sandslash", 29: "nidoran-f", 30: "nidorina",
    31: "nidoqueen", 32: "nidoran-m", 33: "nidorino", 34: "nidoking", 35: "clefairy",
    36: "clefable", 37: "vulpix", 38: "ninetales", 39: "jigglypuff", 40: "wigglytuff",
    41: "zubat", 42: "golbat", 43: "oddish", 44: "gloom", 45: "vileplume",
    46: "paras", 47: "parasect", 48: "venonat", 49: "venomoth", 50: "diglett",
    51: "dugtrio", 52: "meowth", 53: "persian", 54: "psyduck", 55: "golduck",
    56: "mankey", 57: "primeape", 58: "growlithe", 59: "arcanine", 60: "poliwag",
    61: "poliwhirl", 62: "poliwrath", 63: "abra", 64: "kadabra", 65: "alakazam",
    66: "machop", 67: "machoke", 68: "machamp", 69: "bellsprout", 70: "weepinbell",
    71: "victreebel", 72: "tentacool", 73: "tentacruel", 74: "geodude", 75: "graveler",
    76: "golem", 77: "ponyta", 78: "rapidash", 79: "slowpoke", 80: "slowbro",
    81: "magnemite", 82: "magneton", 83: "farfetchd", 84: "doduo", 85: "dodrio",
    86: "seel", 87: "dewgong", 88: "grimer", 89: "muk", 90: "shellder",
    91: "cloyster", 92: "gastly", 93: "haunter", 94: "gengar", 95: "onix",
    96: "drowzee", 97: "hypno", 98: "krabby", 99: "kingler", 100: "voltorb",
    101: "electrode", 102: "exeggcute", 103: "exeggutor", 104: "cubone", 105: "marowak",
    106: "hitmonlee", 107: "hitmonchan", 108: "lickitung", 109: "koffing", 110: "weezing",
    111: "rhyhorn", 112: "rhydon", 113: "chansey", 114: "tangela", 115: "kangaskhan",
    116: "horsea", 117: "seadra", 118: "goldeen", 119: "seaking", 120: "staryu",
    121: "starmie", 122: "mr-mime", 123: "scyther", 124: "jynx", 125: "electabuzz",
    126: "magmar", 127: "pinsir", 128: "tauros", 129: "magikarp", 130: "gyarados",
    131: "lapras", 132: "ditto", 133: "eevee", 134: "vaporeon", 135: "jolteon",
    136: "flareon", 137: "porygon", 138: "omanyte", 139: "omastar", 140: "kabuto",
    141: "kabutops", 142: "aerodactyl", 143: "snorlax", 144: "articuno", 145: "zapdos",
    146: "moltres", 147: "dratini", 148: "dragonair", 149: "dragonite", 150: "mewtwo",
    151: "mew"
]
